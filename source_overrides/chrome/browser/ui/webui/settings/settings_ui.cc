...
#include "chrome/browser/ui/webui/settings/focus_crash_reporting_handler.h"
...
SettingsUI::SettingsUI(content::WebUI* web_ui)
  ...
  AddSettingsPageUIHandler(std::make_unique<FocusCrashReportingHandler>());
  ...
  // Do not expose a Google/Chrome Sync button in this distribution. Focus is
  // built without distribution-owned OAuth credentials, and Google restricts
  // Chrome Sync tokens for third-party Chromium derivatives. Keeping this
  // false avoids presenting a sign-in flow that cannot complete. Local profile
  // storage and the Chrome data importer remain fully available.
  ...
  html_source->AddBoolean("enableAiModeSearchSetting", false);
