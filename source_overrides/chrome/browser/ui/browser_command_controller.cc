...
bool ShouldToggleVerticalCollapse(PrefService* prefs) {
  return tabs::IsVerticalTabsFeatureEnabled() &&
         ...
         prefs->GetInteger(prefs::kFocusLayout) ==
             std::to_underlying(FocusLayoutType::kVertical);
}
...
BrowserCommandController::BrowserCommandController(BrowserWindowInterface* bwi)
  ...
  profile_pref_registrar_.AddMultiple(
      {prefs::kFocusLayout, prefs::kVerticalCollapseShortcut},
...
bool BrowserCommandController::ExecuteCommandWithDisposition(
  ...
  switch (id) {
    ...
    case IDC_BROWSER_LAYOUT_CLASSIC:
      SetBrowserLayout(browser_, FocusLayoutType::kClassic);
      break;
    case IDC_BROWSER_LAYOUT_DYNAMIC:
      SetBrowserLayout(browser_, FocusLayoutType::kDynamic);
      break;
    case IDC_BROWSER_LAYOUT_COMPACT:
      SetBrowserLayout(browser_, FocusLayoutType::kCompact);
      break;
    case IDC_BROWSER_LAYOUT_VERTICAL:
      SetBrowserLayout(browser_, FocusLayoutType::kVertical);
      break;
    ...
    case IDC_UPGRADE_DIALOG:
      ...
      break;
    case IDC_OPEN_MEDITATION:
      ShowMeditation(browser_->GetBrowserForOpeningWebUi());
      break;
    ...
    case IDC_CTRL_S_SHORTCUT: {
      // In zen mode with VTS, Ctrl+S toggles the side chrome visibility
      // instead of collapsing the tab strip.
      PrefService* prefs = profile()->GetPrefs();
      auto* browser_view = BrowserView::GetBrowserViewForBrowser(browser_);
      if (browser_view && browser_view->IsZenModeEnabledForLayout() &&
          browser_view->ShouldDrawVerticalTabStrip() &&
          ShouldToggleVerticalCollapse(prefs)) {
        browser_view->ToggleZenModeSideChrome();
        return true;
      }
    ...
    case IDC_TOGGLE_ZEN_MODE_TOP_CHROME_PIN: {
      if (auto* browser_view =
              BrowserView::GetBrowserViewForBrowser(browser_)) {
        browser_view->ToggleZenModeTopChrome();
        return true;
      }
      break;
    }
...
void BrowserCommandController::InitCommandState() {
  ...
  // Focus Browser layout menu
  command_updater_.UpdateCommandEnabled(IDC_BROWSER_LAYOUT_MENU, true);
  command_updater_.UpdateCommandEnabled(IDC_BROWSER_LAYOUT_CLASSIC, true);
  command_updater_.UpdateCommandEnabled(IDC_BROWSER_LAYOUT_DYNAMIC, true);
  ...
  // Window management commands
  command_updater_.UpdateCommandEnabled(IDC_FORCE_PASTE, true);
  ...
  command_updater_.UpdateCommandEnabled(IDC_OPEN_MEDITATION, true);
  command_updater_.UpdateCommandEnabled(IDC_TOGGLE_ZEN_MODE_TOP_CHROME_PIN,
                                        false);
...
void BrowserCommandController::UpdateCommandsForExtensionsMenu() {
  // TODO(crbug.com/41124423): Talk with isandrk@ch40m1um.qjz9zk about whether this
