...
class VerticalTabStripRegionViewTest
 ...
 protected:
  ...
  void PressCollapseButton() {
    ...
    ASSERT_TRUE(collapse_action);
    collapse_action->InvokeAction();
  }
...
IN_PROC_BROWSER_TEST_F(VerticalTabStripRegionViewTest, ImmersiveModeLock) {
  ...
  // 2. Try to disable vertical tabs through the current layout controller.
  ...
  // 7. Try to enable vertical tabs through the current layout controller.
...
IN_PROC_BROWSER_TEST_F(VerticalTabStripRegionViewTest,
  ...
  ASSERT_TRUE(state_controller()->IsExpandOnHoverEnabled());
...
IN_PROC_BROWSER_TEST_F(VerticalTabStripRegionViewTest, ModeChanged) {
  ...
  ASSERT_FALSE(state_controller()->IsExpandOnHoverEnabled());
  ASSERT_FALSE(region_view()->is_expanded_on_hover());
  ...
  state_controller()->SetVerticalTabsEnabled(false);
