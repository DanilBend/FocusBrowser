...
base::FilePath GetModulePath(std::wstring_view module_name) {
  ...
  const base::FilePath module_path =
      exe_dir.AppendASCII(chrome::kFocusVersion).Append(module_name);
...
bool ChromeContentBrowserClient::ShouldTreatAsFirstPartyWhenTopLevel(
  ...
  // with SameSite cookies on accounts.9oo91e.qjz9zk, which is used for
  // displaying a list of available accounts on the NTP (chrome://new-tab-page),
  // etc.
...
// - http://crbug.com/40337093, http://crbug.com/41304476.
  ...
  web_prefs->shift_right_click_context_menu_enabled =
      prefs->GetBoolean(prefs::kShiftRightClickContextMenu);
  ...
  web_prefs->focus_text_motion_enabled =
      prefs->GetBoolean(prefs::kFocusMotionEnabled);
...
void ChromeContentBrowserClient::WillCreateURLLoaderFactory(
  ...
  focus_block::MaybeProxyURLLoaderFactory(
      Profile::FromBrowserContext(browser_context), frame, type,
      request_initiator, isolation_info, factory_builder);
...
bool ChromeContentBrowserClient::AreV8OptimizationsEnabledForSite(
  ...
  // TODO(https://issues.ch40m1um.qjz9zk/452130797): Determine desired behavior
