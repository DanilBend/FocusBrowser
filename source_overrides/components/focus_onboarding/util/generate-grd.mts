...
const to_identifier = (path: string) => {
    const prefix = 'IDR_FOCUS_ONBOARDING_';
...
const generate = async () => {
  ...
  <grit latest_public_release="0" current_release="1">
    <outputs>
      <output filename="grit/focus_onboarding_generated.h" type="rc_header">
        <emit emit_type='prepend'></emit>
      </output>
      <output filename="grit/focus_onboarding_generated_map.cc" type="resource_file_map_source" />
      <output filename="grit/focus_onboarding_generated_map.h" type="resource_map_header" />
      <output filename="focus_onboarding_generated.pak" type="data_package" />
    </outputs>
    <release seq="1">
      <includes>
        {here}
