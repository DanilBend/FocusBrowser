...
const syncListExpanderStates = ( ) => {
    for ( const expandable of qsa$('#lists .expandable') ) {
        const expander =
            qs$(expandable, ':scope > .listExpander') ||
            qs$(expandable, ':scope > .detailbar .listExpander');
        if ( expander === null ) { continue; }
        expander.setAttribute(
            'aria-expanded',
            dom.cl.has(expandable, 'expanded') ? 'true' : 'false'
        );
        if ( expander.hasAttribute('aria-label') ) { continue; }
        const listName = qs$(expandable, ':scope > .detailbar .listname');
        const label = listName && listName.textContent.trim();
        expander.setAttribute(
            'aria-label',
            label || i18n$('3pListsOfBlockedHostsHeader')
        );
    }
};
...
dom.on('#lists', 'keydown', '.listExpander', ev => {
