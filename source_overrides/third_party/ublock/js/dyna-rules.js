...
const syncDiffCollapseState = isCollapsed => {
    const control = qs$('#ruleFilter #diffCollapse');
    dom.cl.toggle(control, 'active', isCollapsed);
    control.setAttribute('aria-expanded', isCollapsed ? 'false' : 'true');
    control.setAttribute(
        'aria-label',
        i18n$(isCollapsed ? 'popupMoreButton_v2' : 'popupLessButton_v2')
    );
};
...
const onPresentationChanged = (( ) => {
    ...
    dom.on('#ruleFilter #diffCollapse', 'click', ( ) => {
        presentationState.isCollapsed = !presentationState.isCollapsed;
        syncDiffCollapseState(presentationState.isCollapsed);
        savePresentationState();
        onPresentationChanged(true);
    });
    dom.on('#ruleFilter #diffCollapse', 'keydown', ev => {
...
vAPI.localStorage.getItemAsync('dynaRulesPresentationState').then(details => {
    ...
    if ( typeof details.isCollapsed === 'boolean' ) {
        ...
        syncDiffCollapseState(details.isCollapsed);
    }
