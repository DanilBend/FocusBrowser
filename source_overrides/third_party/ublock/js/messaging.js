...
const popupDataFromTabId = function(tabId, tabTitle) {
    ...
    const r = {
        ...
        focusGlobalEnabled: µb.focusGlobalEnabled !== false,
...
const onMessage = function(request, sender, callback) {
    ...
    case 'toggleFocusGlobalFiltering': {
        const enabled = request.state !== false;
        if ( µb.focusGlobalEnabled !== enabled ) {
            µb.focusGlobalEnabled = enabled;
            vAPI.storage.set({ focusGlobalEnabled: enabled });
            filteringBehaviorChanged({ direction: enabled ? 1 : -1 });
        }
        µb.updateToolbarIcon(request.tabId, 0b111);
        response = popupDataFromTabId(request.tabId);
        if ( response.pageURL !== '' && isNetworkURI(response.rawURL) ) {
