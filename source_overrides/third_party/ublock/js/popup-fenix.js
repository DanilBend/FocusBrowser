...
if ( new URLSearchParams(self.location.search).get('focusMotion') === '0' ) {
...
const renderPrivacyExposure = function() {
    ...
    dom.text('#focusDomainsValue', summary);
};

/******************************************************************************/

const updateHnSwitches = function() {
    const setSwitchState = (selector, state) => {
        dom.cl.toggle(selector, 'on', state);
        dom.attr(selector, 'aria-pressed', state ? 'true' : 'false');
    };
    setSwitchState('#no-popups', popupData.noPopups === true);
    setSwitchState('#no-large-media', popupData.noLargeMedia === true);
    setSwitchState(
        '#no-cosmetic-filtering',
        popupData.noCosmeticFiltering === true
    );
    setSwitchState('#no-remote-fonts', popupData.noRemoteFonts === true);
    setSwitchState('#no-scripting', popupData.noScripting === true);
};
...
const renderPopup = function() {
    ...
    dom.attr(
        '#switch',
        'aria-pressed',
        popupData.pageURL !== '' && isFiltering ? 'true' : 'false'
    );
    dom.attr(
        '#switch',
        'aria-checked',
    ...
    dom.text('[data-focus-stat="page"] + span', text);
    ...
    dom.text('[data-focus-stat="total"] + span', text);
...
const toggleNetFilteringSwitch = function(ev) {
    ...
    if ( popupData.focusGlobalEnabled === false ) { return; }
    const isFilteringEnabled =
        dom.cl.toggle(dom.body, 'off') === false;
    dom.attr(
        '#switch',
        'aria-pressed',
        isFilteringEnabled ? 'true' : 'false'
    );
    dom.attr(
        '#switch',
        'aria-checked',
...
const toggleFocusGlobalSwitch = async function() {
    if ( !popupData ) { return; }
    const response = await messaging.send('popupPanel', {
        what: 'toggleFocusGlobalFiltering',
        state: popupData.focusGlobalEnabled === false,
        tabId: popupData.tabId,
    });
...
const gotoURL = function(ev) {
    ...
    if ( anchor === null || anchor.hasAttribute('href') === false ) { return; }
...
const toggleHostnameSwitch = async function(ev) {
    ...
    const state = dom.cl.toggle(target, 'on');
    dom.attr(target, 'aria-pressed', state ? 'true' : 'false');
...
dom.on('#focusGlobalSwitch', 'click', toggleFocusGlobalSwitch);
