...
bool HasNonTrivialSpellingGrammarStyles(const FragmentItem& fragment_item,
  ...
  if (const ComputedStyle* pseudo_style =
    ...
    if (pseudo_style->TextStrokeWidth() !=
        originating_style.TextStrokeWidth()) {
      return true;
    }
      ...
      if (controller.HasAnyMarkersForText(*text_node)) {
        ...
        custom_ = controller.MarkersFor(
            *text_node, DocumentMarker::kCustomHighlight,
            fragment_dom_offsets_->start, fragment_dom_offsets_->end);
        // Marker creation and its animation callback are the authority for
        // the preference and reduced-motion policy. Keeping painting purely
        // marker-driven also avoids rebuilding media-query state per glyph.
        focus_text_motion_ = controller.MarkersFor(
            *text_node, DocumentMarker::kFocusTextMotion,
            fragment_dom_offsets_->start, fragment_dom_offsets_->end);
      } else if (selection) {
        fragment_dom_offsets_ = GetFragmentDOMOffsets(
            *text_node, fragment_paint_info_.from, fragment_paint_info_.to);
...
void HighlightPainter::PaintNonCssMarkers(Phase phase) {
  ...
  for (const DocumentMarker* marker : markers_) {
    ...
    switch (marker->GetType()) {
      ...
      case DocumentMarker::kFocusTextMotion:
        // This marker participates in the originating text paint itself. It
        // has no separate background or foreground pass.
        break;
...
HighlightPainter::Case HighlightPainter::ComputePaintCase() const {
  ...
  // The overlay path already splits shaped text at exact range boundaries and
  // clips partial ligatures. Reusing it avoids any DOM overlay or line-wide
  // transform while preserving site colors, gradients and writing modes.
  if (!focus_text_motion_.empty()) {
    return kOverlay;
  }
...
void HighlightPainter::ApplyFocusTextMotionToParts() {
  if (focus_text_motion_.empty() || parts_.empty()) {
    return;
  }
  const auto* text_node = DynamicTo<Text>(node_);
  if (!text_node || !fragment_dom_offsets_) {
    return;
  }
  ...
  struct MotionRange {
    ...
    unsigned to;
    float opacity;
    float translation_inline;
    float translation_y;
  };
  Vector<MotionRange> motion_ranges;
  MarkerRangeMappingContext mapping_context(*text_node, *layout_object_,
                                            *fragment_dom_offsets_);
  for (const DocumentMarker* marker : focus_text_motion_) {
    const std::optional<TextOffsetRange> offsets =
        mapping_context.GetTextContentOffsets(*marker);
    if (!offsets || offsets->start >= offsets->end) {
      continue;
    }
    const auto& motion = To<FocusTextMotionMarker>(*marker);
    motion_ranges.push_back(
        MotionRange{offsets->start, offsets->end, motion.Opacity(),
                    motion.TranslationInline(), motion.TranslationY()});
  }
  if (motion_ranges.empty()) {
    return;
  }

  HeapVector<HighlightPart> split_parts;
  for (const HighlightPart& part : parts_) {
    Vector<unsigned> boundaries = {part.range.from, part.range.to};
    for (const MotionRange& motion : motion_ranges) {
      if (motion.to <= part.range.from || motion.from >= part.range.to) {
        continue;
      }
      boundaries.push_back(std::max(part.range.from, motion.from));
      boundaries.push_back(std::min(part.range.to, motion.to));
    }
    std::sort(boundaries.begin(), boundaries.end());
    const auto unique_end = std::unique(boundaries.begin(), boundaries.end());
    boundaries.resize(static_cast<wtf_size_t>(unique_end - boundaries.begin()));

    for (wtf_size_t i = 1; i < boundaries.size(); ++i) {
      const unsigned from = boundaries[i - 1];
      const unsigned to = boundaries[i];
      if (from >= to) {
        continue;
      }
      const unsigned probe = from + (to - from) / 2;
      float opacity = 1.0f;
      float translation_inline = 0.0f;
      float translation_y = 0.0f;
      for (const MotionRange& motion : motion_ranges) {
        if (probe >= motion.from && probe < motion.to) {
          // Overlapping edits use the least advanced reveal, so the newest
          // insertion never flashes fully opaque or jumps to its baseline for
          // one frame.
          opacity = std::min(opacity, motion.opacity);
          translation_inline =
              std::max(translation_inline, motion.translation_inline);
          translation_y = std::max(translation_y, motion.translation_y);
        }
      }
      HighlightPart split = part;
      split.range = HighlightOverlay::HighlightRange(from, to);
      split.opacity = opacity;
      const float signed_inline_translation =
          originating_style_.Direction() == TextDirection::kRtl
              ? -translation_inline
              : translation_inline;
      if (originating_style_.IsHorizontalWritingMode()) {
        split.translation_x = signed_inline_translation;
        split.translation_y = translation_y;
      } else {
        split.translation_y = translation_y + signed_inline_translation;
      }
      split_parts.push_back(std::move(split));
    }
  }
  parts_ = std::move(split_parts);
}
  ...
  for (auto& part : parts_) {
    ...
    if (fragment_paint_info_.shape_result) {
      ...
      if (fragment_item_.IsSvgText() &&
        ...
        const float absolute_translation_x = part.translation_x < 0.0f
                                                 ? -part.translation_x
                                                 : part.translation_x;
        const float absolute_translation_y = part.translation_y < 0.0f
                                                 ? -part.translation_y
                                                 : part.translation_y;
        const float maximum_translation =
            std::max(absolute_translation_x, absolute_translation_y);
        if (maximum_translation > 0.0f) {
          clip_rect.Inflate(LayoutUnit::FromFloatCeil(maximum_translation));
        }
