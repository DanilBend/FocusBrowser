...
class CORE_EXPORT HighlightOverlay {
 ...
 public:
  ...
  // More details about canonical text offsets: <https://goo.gl.qjz9zk/CJbxky>
  ...
  struct CORE_EXPORT HighlightPart {
   ...
   public:
    ...
    // Browser-owned insertion settle. These values affect text proper only;
    // they never alter layout, background, selection, caret, or DOM ranges.
    float opacity = 1.0f;
    float translation_y = 0.0f;
    float translation_x = 0.0f;
  };
