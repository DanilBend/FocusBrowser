...
bool FocusTextMotionPrefersReducedMotion(const Document& document) {
...
DocumentMarker::MarkerTypeIndex MarkerTypeToMarkerIndex(
  ...
  switch (type) {
    ...
    case DocumentMarker::kFocusTextMotion:
      return DocumentMarker::kFocusTextMotionMarkerIndex;
  }
...
DocumentMarkerList* CreateListForType(DocumentMarker::MarkerType type) {
  switch (type) {
    ...
    case DocumentMarker::kFocusTextMotion:
      return MakeGarbageCollected<FocusTextMotionMarkerListImpl>();
  }
...
class FocusTextMotionFrameCallback final : public FrameCallback {
 public:
  explicit FocusTextMotionFrameCallback(
      DocumentMarkerController* marker_controller)
      : marker_controller_(marker_controller) {}

  void Invoke(double high_res_ms) override {
    marker_controller_->ContinueFocusTextMotionAnimation(
...
void DocumentMarkerController::AddFocusTextMotionMarker(
    const Text& text,
    unsigned start_offset,
    unsigned end_offset,
    base::TimeDelta start_delay) {
  CHECK(document_);
  if (start_offset >= end_offset || end_offset > text.length() ||
      !document_->GetSettings()->GetFocusTextMotionEnabled() ||
      FocusTextMotionPrefersReducedMotion(*document_)) {
    return;
  }

  AddMarkerToNode(text,
                  MakeGarbageCollected<FocusTextMotionMarker>(
                      start_offset, end_offset,
                      FocusTextMotionMarker::Kind::kInsertion, start_delay));
  StartFocusTextMotionAnimationIfNeeded();
}

void DocumentMarkerController::AddFocusTextDeletionMotionMarker(
    const Text& text,
    unsigned start_offset,
    unsigned end_offset) {
  CHECK(document_);
  ...
  AddMarkerToNode(text, MakeGarbageCollected<FocusTextMotionMarker>(
                            start_offset, end_offset,
                            FocusTextMotionMarker::Kind::kDeletionSettle));
  StartFocusTextMotionAnimationIfNeeded();
}
...
void DocumentMarkerController::DidRemoveNodeFromMap(
  ...
  if (type == DocumentMarker::kFocusTextMotion) {
    focus_text_motion_animation_running_ = false;
  }
}
...
void DocumentMarkerController::StartFocusTextMotionAnimationIfNeeded() {
  CHECK(document_);
  if (focus_text_motion_animation_running_ ||
      !PossiblyHasMarkers(DocumentMarker::kFocusTextMotion)) {
    return;
  }
  focus_text_motion_animation_running_ = true;
  document_->RequestAnimationFrame(
      MakeGarbageCollected<FocusTextMotionFrameCallback>(this));
}

void DocumentMarkerController::ContinueFocusTextMotionAnimation(
    base::TimeTicks tick) {
  CHECK(document_);
  if (!PossiblyHasMarkers(DocumentMarker::kFocusTextMotion)) {
    focus_text_motion_animation_running_ = false;
    return;
  }

  if (!document_->GetSettings()->GetFocusTextMotionEnabled() ||
      FocusTextMotionPrefersReducedMotion(*document_)) {
    RemoveMarkersOfTypes(DocumentMarker::MarkerTypes::FocusTextMotion());
    focus_text_motion_animation_running_ = false;
    return;
  }

  const bool all_finished = UpdateFocusTextMotionOpacity(tick);
  InvalidatePaintForFocusTextMotionMarkers();
  if (all_finished) {
    RemoveMarkersOfTypes(DocumentMarker::MarkerTypes::FocusTextMotion());
    focus_text_motion_animation_running_ = false;
    return;
...
bool DocumentMarkerController::UpdateFocusTextMotionOpacity(
    base::TimeTicks tick) {
  CHECK(PossiblyHasMarkers(DocumentMarker::kFocusTextMotion));
  MarkerMap* marker_map =
      markers_[MarkerTypeToMarkerIndex(DocumentMarker::kFocusTextMotion)];
  CHECK(marker_map);
  bool all_finished = true;
  for (auto& [text_node, marker_list] : *marker_map) {
    CHECK_EQ(marker_list->MarkerType(), DocumentMarker::kFocusTextMotion);
    for (DocumentMarker* marker : marker_list->GetMarkers()) {
      all_finished &= To<FocusTextMotionMarker>(marker)->UpdateOpacity(tick);
    }
  }
  return all_finished;
}

void DocumentMarkerController::InvalidatePaintForFocusTextMotionMarkers() {
  CHECK(PossiblyHasMarkers(DocumentMarker::kFocusTextMotion));
  MarkerMap* marker_map =
      markers_[MarkerTypeToMarkerIndex(DocumentMarker::kFocusTextMotion)];
  CHECK(marker_map);
