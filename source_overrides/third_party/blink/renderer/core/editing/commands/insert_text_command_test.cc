...
TEST_F(InsertTextCommandTest, FocusTextMotionMarksOnlyInsertedRange) {
  GetDocument().GetSettings()->SetFocusTextMotionEnabled(true);
  Selection().SetSelection(
      SetSelectionTextToBody("<p contenteditable>ab|cd</p>"),
      SetSelectionOptions());

  GetDocument().execCommand("insertText", false, "X", ASSERT_NO_EXCEPTION);

  auto* text = To<Text>(GetDocument().body()->firstChild()->firstChild());
...
TEST_F(InsertTextCommandTest, FocusTextMotionHonorsReducedMotion) {
  GetDocument().GetSettings()->SetFocusTextMotionEnabled(true);
...
TEST_F(InsertTextCommandTest, FocusTextMotionSkipsProvisionalImeUpdate) {
  GetDocument().GetSettings()->SetFocusTextMotionEnabled(true);
  Selection().SetSelection(
      SetSelectionTextToBody("<p contenteditable>ab|cd</p>"),
      SetSelectionOptions());

  auto* command = MakeGarbageCollected<InsertTextCommand>(
