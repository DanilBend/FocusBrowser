...
void InsertIncrementalTextCommand::DoApply(EditingState* editing_state) {
  ...
  // Rich-editable IME confirmation commonly replaces the active composition
  // with byte-for-byte identical text. Incremental insertion then has no DOM
  // delta to mark, even though this is the first committed (non-provisional)
  // paint. Mark the existing composition range so contenteditable receives the
  // same settle motion as input and textarea without animating IME updates.
  if (enable_focus_text_motion_ && !old_text.empty() && old_text == new_text) {
    for (TextIterator marked_text(selection_range.StartPosition(),
                                  selection_range.EndPosition());
         !marked_text.AtEnd(); marked_text.Advance()) {
      auto* text_node = DynamicTo<Text>(marked_text.CurrentContainer());
      if (!text_node) {
        continue;
      }
      const unsigned start = marked_text.StartOffsetInCurrentContainer();
      const unsigned end = marked_text.EndOffsetInCurrentContainer();
      if (end <= start) {
        continue;
      }
      AddFocusInsertionMotionMarkers(
          *text_node, start, text_node->data().substr(start, end - start));
    }
  }
