...
def a_all_patches_in_tree_are_in_series():
    for patch in patches_dir.rglob('*'):
        ...
        assert patch.relative_to(patches_dir).as_posix() in series, \
...
def c_all_new_files_have_license_header():
    for patch in series:
        if not patch.startswith(FOCUS_PATCH_PREFIX):
            continue
        ...
        for file in added_files:
            ...
            assert any('terms of the GPL-3.0 license' in str(hunk) for hunk in file), \
                   f"File {file.path} was added in {patch}, but contains no Focus Browser license header"
