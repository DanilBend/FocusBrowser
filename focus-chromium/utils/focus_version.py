...
"""
Script to generate Focus Browser version numbers
...
# Focus Browser 1.0 is based on Chromium 150. Future Chromium major updates
# continue to advance the Focus minor version without changing Chromium's own
# four-part version.
CHROME_VERSION_BASE = 150
...
def check_existing_version(path):
    ...
    with open(path, "r", encoding="utf-8") as file:
        if "FOCUS" in file.read():
            raise ValueError("file already contains Focus Browser versioning")
...
def get_version_parts(tree, platform_tree):
    ...
    version_paths = {
        "FOCUS_MAJOR": tree / "version.txt",
        "FOCUS_MINOR": tree / "chromium_version.txt",
        "FOCUS_PATCH": tree / "revision.txt",
    }

    if platform_tree is not None:
        version_paths["FOCUS_PLATFORM"] = platform_tree / "revision.txt"

    version_parts = {}
    for name, path in version_paths.items():
        delta = 0 if name != "FOCUS_MINOR" else -CHROME_VERSION_BASE
